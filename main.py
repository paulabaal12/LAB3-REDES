...
async def on_message(msg):
    ...
    async with LOCK:
        if msg_type == "hello":
            if from_node in STATIC_NEIGHBORS:
                # refrescar o reaprender
                if from_node not in TOPOLOGY[NODE_ID]:
                    TOPOLOGY[NODE_ID][from_node] = {"weight": msg["hops"], "time": 60}
                else:
                    TOPOLOGY[NODE_ID][from_node]["time"] = 60

                if NODE_ID not in TOPOLOGY[from_node]:
                    TOPOLOGY[from_node][NODE_ID] = {"weight": msg["hops"], "time": 60}
                else:
                    TOPOLOGY[from_node][NODE_ID]["time"] = 60

                print(f"[HELLO] Refrescada conexión {NODE_ID} <-> {from_node}")
            else:
                print(f"[HELLO] Ignorado hello de {from_node} (no es vecino fijo)")
            ...
            if to_node not in TOPOLOGY[from_node]:
                TOPOLOGY[from_node][to_node] = {"weight": hops, "time": 60}
                TOPOLOGY[to_node][from_node] = {"weight": hops, "time": 60}
                print(f"{Fore.GREEN}[MSG]{Style.RESET_ALL} Aprendida nueva conexión {from_node} <-> {to_node}")
            else:
                TOPOLOGY[from_node][to_node]["time"] = 60
                TOPOLOGY[to_node][from_node]["time"] = 60
...
async def send_messages():
    ...
    while True:
        ...
        async with LOCK:
            # Recorremos TODAS las conexiones conocidas en la topología
            for node, neighbors in TOPOLOGY.items():
                for nbr, data in neighbors.items():
                    # Difundir la conexión node <-> nbr a TODOS mis vecinos
                    for my_nbr in TOPOLOGY[NODE_ID].keys():
                        msg = make_message(node, nbr, data["weight"])
                        await TRANSPORT.send(my_nbr, msg)
                        print(f"{Fore.YELLOW}[UPDATE]{Style.RESET_ALL} Difundiendo {node} <-> {nbr} a {my_nbr}")
...
async def menu_loop():
    ...
    while True:
        ...
        if opcion.strip() == "1":
            ...
            dijkstra.printTable()
            dijkstra.fill_csv()
            print("Dijkstra calculado en output/single_table.txt y output/table.csv")
        elif opcion.strip() == "3":
...
async def main():
    global TOPOLOGY, NODE_ID, TRANSPORT, STATIC_NEIGHBORS
    ...
    STATIC_NEIGHBORS = set(neighbors.keys())

    TOPOLOGY[NODE_ID] = {}
    for nbr, data in neighbors.items():
        TOPOLOGY[NODE_ID][nbr] = {"weight": data["weight"], "time": 10}
        if nbr not in TOPOLOGY:
            TOPOLOGY[nbr] = {}
        TOPOLOGY[nbr][NODE_ID] = {"weight": data["weight"], "time": 10}
